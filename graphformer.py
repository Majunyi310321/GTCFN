...
class GraphFormerConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, num_heads, kernel_transformation=softmax_kernel_transformation, projection_matrix_type='a',
                 nb_random_features=10, use_gumbel=True, nb_gumbel_sample=10, rb_order=1, rb_trans='sigmoid', use_edge_loss=True):
        super(GraphFormerConv, self).__init__()
...
class GraphFormer(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=3, num_heads=6, dropout=0.0,
                 kernel_transformation=softmax_kernel_transformation, nb_random_features=30, use_bn=True, use_gumbel=True,
                 use_residual=True, use_act=False, use_jk=False, nb_gumbel_sample=10, rb_order=1, rb_trans='sigmoid', use_edge_loss=True):
        ...
        for i in range(num_layers):
            self.convs.append(
                GraphFormerConv(hidden_channels, hidden_channels, num_heads=num_heads, kernel_transformation=kernel_transformation,
